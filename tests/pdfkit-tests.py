...
class TestPDFKitCommandGeneration(unittest.TestCase):
    ...
    def test_command_construction(self):
        r = pdfkit.PDFKit(
            "html", "string", options={"page-size": "Letter", "toc-l1-font-size": 12}
        )
        command = r.command()
    ...
    def test_toc_with_options(self):
        ...
        r = pdfkit.PDFKit(
            "html", "string", options=options, toc={"xsl-style-sheet": "test.xsl"}
        )
    ...
    def test_cover_and_toc_cover_first(self):
        ...
        self.assertEqual(
            command[-7:],
            ["cover", "test.html", "toc", "--xsl-style-sheet", "test.xsl", "-", "-"],
        )
...
class TestPDFKitGeneration(unittest.TestCase):
    ...
    def test_pdf_generation_into_variable(self):
        r = pdfkit.PDFKit("html", "string", options={"page-size": "Letter"})
        pdf = r.to_pdf()
        self.assertTrue(pdf[0:4].decode("ascii").startswith("%PDF"))
